...
ENT.JumpSound = Sound("npc_ANGRY_MUNCI/jump.mp3")
ENT.JumpHighSound = Sound("npc_ANGRY_MUNCI/jump.mp3")
ENT.TauntSounds = {
	Sound("npc_ANGRY_MUNCI/DEATH2.mp3"),
	Sound("npc_ANGRY_MUNCI/DEATH1.mp3"),
	Sound("npc_ANGRY_MUNCI/DEATH3.mp3"),
	Sound("npc_ANGRY_MUNCI/DEATH4.mp3"),
}
local chaseMusic = Sound("npc_ANGRY_MUNCI/panic.mp3")

local IsValid = IsValid

if SERVER then -- SERVER --

local npc_ANGRY_MUNCI_acquire_distance =
	CreateConVar("npc_ANGRY_MUNCI_acquire_distance", 125000, FCVAR_NONE,
	"The maximum distance at which ANGRY_MUNCI will chase a target.")

local npc_ANGRY_MUNCI_spawn_protect =
	CreateConVar("npc_ANGRY_MUNCI_spawn_protect", 2, FCVAR_NONE,
	"If set to 1, ANGRY_MUNCI will not target players or hide within 200 units of \z
	a spawn point.")

local npc_ANGRY_MUNCI_attack_distance =
	CreateConVar("npc_ANGRY_MUNCI_attack_distance", 80, FCVAR_NONE,
	"The reach of ANGRY_MUNCI's attack.")

local npc_ANGRY_MUNCI_attack_interval =
	CreateConVar("npc_ANGRY_MUNCI_attack_interval", 0.2, FCVAR_NONE,
	"The delay between ANGRY_MUNCI's attacks.")

local npc_ANGRY_MUNCI_attack_force =
	CreateConVar("npc_ANGRY_MUNCI_attack_force", 1750, FCVAR_NONE,
	"The physical force of ANGRY_MUNCI's attack. Higher values throw things \z
	farther.")

local npc_ANGRY_MUNCI_smash_props =
	CreateConVar("npc_ANGRY_MUNCI_smash_props", 1, FCVAR_NONE,
	"If set to 1, ANGRY_MUNCI will punch through any props placed in their way.")

local npc_ANGRY_MUNCI_allow_jump =
	CreateConVar("npc_ANGRY_MUNCI_allow_jump", 1, FCVAR_NONE,
	"If set to 1, ANGRY_MUNCI will be able to jump.")

local npc_ANGRY_MUNCI_hiding_scan_interval =
	CreateConVar("npc_ANGRY_MUNCI_hiding_scan_interval", 10, FCVAR_NONE,
	"ANGRY_MUNCI will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant ANGRY_MUNCIs are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_ANGRY_MUNCI_hiding_repath_interval =
	CreateConVar("npc_ANGRY_MUNCI_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to ANGRY_MUNCI's hiding spot will be redetermined every X seconds.")

local npc_ANGRY_MUNCI_chase_repath_interval =
	CreateConVar("npc_ANGRY_MUNCI_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of ANGRY_MUNCI's target will be redetermined every \z
	X seconds.")

local npc_ANGRY_MUNCI_expensive_scan_interval =
	CreateConVar("npc_ANGRY_MUNCI_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_ANGRY_MUNCI_force_download =
	CreateConVar("npc_ANGRY_MUNCI_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download ANGRY_MUNCI resources \z
...
util.AddNetworkString("ANGRY_MUNCI_nag")
util.AddNetworkString("ANGRY_MUNCI_navgen")
...
local VECTOR_ANGRY_MUNCI_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_ANGRY_MUNCI_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_ANGRY_MUNCI: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other ANGRY_MUNCIs, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_ANGRY_MUNCI"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "ANGRY_MUNCIMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_ANGRY_MUNCI" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why ANGRY_MUNCI isn't working.
	net.Start("ANGRY_MUNCI_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_ANGRY_MUNCI: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_ANGRY_MUNCI: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_ANGRY_MUNCI: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_ANGRY_MUNCI: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_ANGRY_MUNCI: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, ANGRY_MUNCI in pairs(ents.FindByClass("npc_ANGRY_MUNCI")) do
		ANGRY_MUNCI:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "ANGRY_MUNCINavGen", navEndGenerate)
	else
		print("npc_ANGRY_MUNCI: nav_generate failed to initialize")
...
concommand.Add("npc_ANGRY_MUNCI_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_ANGRY_MUNCI: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. ANGRY_MUNCI doesn't use
	...
	elseif isConsole then
		print("npc_ANGRY_MUNCI: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:Initialize()
	...
	self.loco:SetDesiredSpeed(game.SinglePlayer() and 2500 or 2000)
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_ANGRY_MUNCI_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_ANGRY_MUNCI_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_ANGRY_MUNCI_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_ANGRY_MUNCI_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. ANGRY_MUNCI means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_ANGRY_MUNCI_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_ANGRY_MUNCI_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_ANGRY_MUNCI_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_ANGRY_MUNCI_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_ANGRY_MUNCI_attack_distance:GetInt()
		...
		local repathInterval = npc_ANGRY_MUNCI_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_ANGRY_MUNCI_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_ANGRY_MUNCI_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_ANGRY_MUNCI_hiding_repath_interval:GetFloat()
...
local MAT_ANGRY_MUNCI = Material("npc_ANGRY_MUNCI/ANGRY_MUNCI")
killicon.Add("npc_ANGRY_MUNCI", "npc_ANGRY_MUNCI/killicon", color_white)
language.Add("npc_ANGRY_MUNCI", "Angry Munci")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_ANGRY_MUNCI: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a ANGRY_MUNCI.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_ANGRY_MUNCI_music_volume = 
	CreateConVar("npc_ANGRY_MUNCI_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by ANGRY_MUNCI. (0-1, where 0 is muted)")

-- If another ANGRY_MUNCI comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, ANGRY_MUNCIs do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_ANGRY_MUNCI_panic_COUNT ANGRY_MUNCIs are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of ANGRY_MUNCI.
local MUSIC_ANGRY_MUNCI_PANIC_COUNT = 8

local MUSIC_ANGRY_MUNCI_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_ANGRY_MUNCI_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_ANGRY_MUNCI") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("ANGRY_MUNCIPanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_ANGRY_MUNCI_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_ANGRY_MUNCI" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_ANGRY_MUNCI_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear ANGRY_MUNCI taunt us.
...
local function startTimer()
	if not timer.Exists("ANGRY_MUNCIPanicMusicUpdate") then
		timer.Create("ANGRY_MUNCIPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_ANGRY_MUNCI)
...
local dlight = DynamicLight( self:EntIndex() )
	...
	-- Get the normal vector from ANGRY_MUNCI to the player's eyes, and then compute
	...
	-- ANGRY_MUNCI should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("ANGRY_MUNCIHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("ANGRY_MUNCIHUDSmall", {
	font = "Arial",
	size = 24
})
...
local flavourTexts = {
	{
		...
		"YOU MAY RUN FAST BUT I AM FASTER",
		"I WILL ALWAYS CATCH UP",
	}, {
		"I WILL FIND YOU",
		"I ALWAYS KNOW WHERE YOUR HIDING"
	}, {
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("ANGRY_MUNCI is studying this map.", "ANGRY_MUNCIHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "ANGRY_MUNCIHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "ANGRY_MUNCIHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "ANGRY_MUNCIHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "ANGRY_MUNCIHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("ANGRY_MUNCI_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "ANGRY_MUNCINavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. ANGRY MUNCI doesn't even know where to start with \z
		this map.\n\z
		If you're not running the Sandbox gamemode, switch to that and try \z
		again.", "Error!Error!Error!Error!Error!Error!")
	end
end)
...
local function requestNavGenerate()
	RunConsoleCommand("npc_ANGRY_MUNCI_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for ANGRY MUNCI to figure \z
		this map out.\n\z
		...
		"sacrifice", requestNavGenerate,
		"sacrifice later", nil)
end

net.Receive("ANGRY_MUNCI_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! ANGRY MUNCI doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet ANGRY MUNCI-compatible!",
			"GIVE SOUL", navWarning,
			"sacrifice later", nil,
			"no sacrifice", stopNagging)
	else
		Derma_Query("Uh oh! ANGRY MUNCI doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to ANGRY_MUNCI.\n\z
			\n\z
			If you ARE the server host, you can run npc_ANGRY_MUNCI_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not ANGRY MUNCI-compatible!",
			"sacrifice later", nil,
			"no sacrifice", stopNagging)
	end
end)
...
--
list.Set("NPC", "npc_ANGRY_MUNCI", {
	Name = "Angry Munci",
	Class = "npc_ANGRY_MUNCI",
	Category = "Ultimate Nextbots Pack",
	AdminOnly = false
})
